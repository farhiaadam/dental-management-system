    ...
    <nav className="absolute top-0 left-0 w-full z-50 font-sans text-white bg-gradient-to-b from-black/60 to-transparent px-4 sm:px-8 md:px-12 py-3">
      ...
      <div className="flex justify-between items-center pt-2">
        ...
        <div className="hidden lg:flex items-center gap-6 text-sm font-medium">
          <NavLink 
            to="/" 
            className={({ isActive }) => `hover:text-[#5b58a6] transition py-2 ${isActive ? 'text-[#5b58a6] font-bold' : ''}`}
          >
            Home
          </NavLink>
          ...
          {/* Dropdown Services Selector Context */}
          <div className="relative group cursor-pointer py-2">
            <span className="hover:text-[#5b58a6] transition flex items-center gap-1">
          ...
          {/* Desktop Appointment Overlay Trigger Button */}
          <BookingButton variant="normal" />
        </div>

        {/* Mobile Toggle Burger Trigger Button */}
      ...
      <div className={`fixed top-0 right-0 h-full w-[260px] bg-neutral-900/95 backdrop-blur-md shadow-2xl z-40 p-6 pt-24 transform transition-transform duration-300 ease-in-out ${state.isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col gap-5 text-base font-medium">
          <NavLink to="/" onClick={() => dispatch({ type: 'TOGGLE_MOBILE_MENU' })} className="hover:text-[#5b58a6] py-2 border-b border-white/5">Home</NavLink>
          <NavLink to="/about-us" onClick={() => dispatch({ type: 'TOGGLE_MOBILE_MENU' })} className="hover:text-[#5b58a6] py-2 border-b border-white/5">About Us</NavLink>
          <NavLink to="/services" onClick={() => dispatch({ type: 'TOGGLE_MOBILE_MENU' })} className="hover:text-[#5b58a6] py-2 border-b border-white/5">Services</NavLink>
          <NavLink to="/our-team" onClick={() => dispatch({ type: 'TOGGLE_MOBILE_MENU' })} className="hover:text-[#5b58a6] py-2 border-b border-white/5">Our Team</NavLink>
          <NavLink to="/contact-us" onClick={() => dispatch({ type: 'TOGGLE_MOBILE_MENU' })} className="hover:text-[#5b58a6] py-2 border-b border-white/5">Contact Us</NavLink>
          
          {/* Mobile Floating Action Button Wrapper */}
      ...
      {/* Persistent Floating Side Action Hook Trigger */}
